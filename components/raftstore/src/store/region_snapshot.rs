    ...
    S: Snapshot,
{
    ...
    fn get_value_p_cf_range_opt(
        ...
        start_key: &[u8],
        end_key: &[u8],
    ) -> EngineResult<Option<Vec<Self::DBVector>>> {
        check_key_in_range(
            start_key,
            self.region.get_id(),
            self.region.get_start_key(),
        ...
        self.snap
            .get_value_p_cf_range_opt(opts, cf, &data_start_key, &data_end_key)
            .map_err(|e| self.handle_get_value_error(e, cf, &data_start_key))
    }
}
